...
type VoiceChannel = 'nyx' | 'hex' | 'mani';
...
const VOICE_SHORTCUTS: Record<VoiceChannel, string> = {
  nyx: 'Alt+N', hex: 'Alt+H', mani: 'Alt+M',
};

const VOICE_COLORS: Record<VoiceChannel, string> = {
  nyx: '#c084fc', hex: '#86efac', mani: '#67e8f9',
};
...
// ─── Style tokens (mirror existing page) ─────────────────────────────────────
    ...
    <div style={{
      ...
      {lastVoice && (
        <span style={{ color: 'var(--muted)' }}>
          last: <span style={{ color: 'var(--accent)' }}>{lastVoice}</span>
        </span>
      )}
      {pendingCount > 0 && (
        <span style={{
...
function VoicePanel({ voice, onVoiceUsed }: { voice: typeof VOICES[number]; onVoiceUsed: (v: VoiceChannel) => void }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem' }}>
        ...
        <span style={{ ...mono, color, textTransform: 'uppercase', letterSpacing: '0.1em' }}>{voice.label}</span>
      ...
      <div ref={histRef} style={{
        ...
        gap: '0.5rem', maxHeight: 200, paddingRight: '0.25rem',
      }}>
        {history.length === 0
    ...
    <>
      <section style={{ marginBottom: '3rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '1rem' }}>
          {VOICES.map(v => <VoicePanel key={v.key} voice={v} onVoiceUsed={onVoiceUsed} />)}
        </div>
      </section>
